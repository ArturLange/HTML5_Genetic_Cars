...
function updateState(constants, worldConstruct, state) {
    ...
    // console.log(state);
    // check health
    const position = worldConstruct.chassis.GetPosition();
    ...
    if (position.x > state.maxPositionx + 0.02) {
        nextState.health = constants.maxCarHealth;
        return nextState;
    }
...
function calculateScore(state, constants) {
    const avgspeed = (state.maxPositionx / state.frames) * constants.box2dfps;
