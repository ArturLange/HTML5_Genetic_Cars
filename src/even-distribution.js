...
module.exports = {
  ...
  createMutatedClone(schema, generator, parent, factor) {
    return Object.keys(schema).reduce((clone, key) => {
      ...
      switch (schemaProp.type) {
        case 'shuffle' : values = random.mutateShuffle(schemaProp, generator, parent[key], factor,
        ); break;
        case 'float' : values = random.mutateFloats(schemaProp, generator, parent[key], factor,
        ); break;
        case 'integer': values = random.mutateIntegers(schemaProp, generator, parent[key], factor,
        ); break;
        default:
          throw new Error(`Unknown type ${schemaProp.type} of schema for key ${key}`);
      }
